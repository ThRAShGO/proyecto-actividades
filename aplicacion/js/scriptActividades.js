...
(function() {
    ...
    // ajax('index.php', {ruta: 'ajaxActividades', accion: 'actividadesPage'}, function (json) {
    //     login(json);
    // });
    
    // function ajax(url, data, responseFunction) {
    ...
    function mostrarActividades (objetoJson){
        objetoJson.actividades[0].titulo;
        for(var i = 0; i < objetoJson.actividades.length; ++i){
            $('#divRowUsuarios').append(getTextActividad(objetoJson.actividades[i]));
        }
        //addEventToPagesLinks();
    }
    ...
    function getTextActividad(actividad){
        var s = 
        '<div class="col-md-4">'+
            ...
            '<p>'+actividad.email+'</p>'+
            '<p>'+actividad.idGrupo+'</p>'+
            '<p>'+actividad.foto+'</p>'+
            '<p><a class="btn btn-default" href="#" role="button">Editar &raquo;</a></p>'+
            '<p><a class="btn btn-default delete" href="#" data-user="'+ actividad.idActividad +'" data-toggle="modal" data-target="#modal-delete" role="button">Borrar &raquo;</a></p>'+
         '</div>';
         return s;
    }
